...
exports.book_detail = function (req, res, next) {
  async.parallel({
    ...
    if (results.book === null) { // No results
      let notFoundErr = new Error('Book not found');
...
exports.book_delete_post = function (req, res, next) {
  async.parallel({
    ...
    if (results.book_instances.length > 0) {
      // Book still has book instances. Re-render as if GET
      res.render('book_delete', {title: 'Delete Book', book: results.book, book_instances: results.book_instances});
    } else {
      // No book instances. Delete book and redirect to book listing
...
exports.book_update_get = function (req, res, next) {
  ...
  async.parallel({
    ...
    // Mark our selected genres as checked.
    for (let i = 0; i < results.genres.length; i++) {
      if (results.book.genre.map(function (g) {
        return g._id.toString()
      }).indexOf(results.genres[i]._id.toString()) > -1) {
        results.genres[i].checked = 'true';
      }
    }
    res.render('book_form', {
